...
#### Preamble ####
# Purpose: Explore the data and make some graphs that we might use in the paper.
# Author: Weiyang Li
...
#   - Ensure the datasets have been downloaded in working directory 
#     '../data/analysis_data/' folder.
#   - A basic understanding of data visualization techniques using ggplot2.
# Additional Information:
#   - The graphs replicated here are based on the analysis of TTC subway delay 
#     data, showcasing key trends and distributions.
#   - Ensure the dataset used matches the structure of the expected data for 
#     accurate replication of graphs.
...
p1 <- ggplot(data_delay, aes(x = min_delay)) +
  geom_histogram(binwidth = 10, fill = "lightpink1", color = "black", 
                 alpha = 0.7) +
...
p2 <- ggplot(data_delay, aes(x = min_gap)) +
  geom_histogram(binwidth = 10, fill = "lightsalmon1", color = "black", 
                 alpha = 0.7) +
