...
#### Preamble ####
# Purpose: Tests the simulated TTC subway delay dataset for structure, 
...
#   - Simulated dataset saved as 'simulated_ttc_delay_data.csv' in the 
#     'data/simulated_data/' directory.
#   - Basic understanding of the data structure and types for TTC subway delay 
#     data.
# Additional Information:
#   - Tests include checking the structure, valid ranges, and logical 
#     consistency of the simulated data.
...
test_that("Data has correct columns", {
  expected_columns <- c("date", "time", "day", "station", "code", "min_delay", 
                        "min_gap", "bound", "line", "vehicle")
  expect_equal(colnames(simulated_data), expected_columns)
})

test_that("Data types are correct", {
  expect_type(simulated_data$date, "character") 
  expect_type(simulated_data$time, "character")
  expect_type(simulated_data$min_delay, "integer")  
  expect_type(simulated_data$min_gap, "integer")
})

# 2. Test value ranges
test_that("Delay and gap values are within valid range", {
  expect_true(all(simulated_data$min_delay >= 1 
                  & simulated_data$min_delay <= 20))
  expect_true(all(simulated_data$min_gap >= 5 
                  & simulated_data$min_gap <= 20))
})

test_that("Station values are valid", {
  valid_stations <- c("DUNDAS STATION", "KENNEDY BD STATION", "BLOOR STATION", 
...
test_that("No missing values in important columns", {
  important_columns <- c("date", "time", "station", "code", "min_delay", 
                         "min_gap", "bound", "line", "vehicle")
